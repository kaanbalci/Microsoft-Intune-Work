...
.PARAMETER EncryptionMethod
    ...
    NOTE: "Auto" means the script will NOT pass -EncryptionMethod to Enable-BitLocker, allowing OS/MDM/GPO policy
    to choose the modern/default method (typically XTS-AES). Legacy AES-CBC modes are intentionally not exposed.
...
.NOTES
    ...
    Created:     2019-10-29
    Updated:     2020-01-13
    Updated:     Updates made by Kaan Balci 2023
    Updated:     Auto EncryptionMethod support + avoid legacy cipher forcing (2025-12-30)
    ...
    1.0.3 - (2025-12-30) Added "Auto" encryption method (omit -EncryptionMethod) and removed legacy AES-CBC options from parameter set
#>

[CmdletBinding(SupportsShouldProcess = $true)]
param(
    [parameter(Mandatory = $false, HelpMessage = "Define the encryption method to be used when enabling BitLocker. 'Auto' omits -EncryptionMethod and lets OS/MDM/GPO choose (recommended).")]
    [ValidateNotNullOrEmpty()]
    # CHANGES MADE:
    # - Added "Auto"
    # - Removed "Aes128" and "Aes256" from allowed values to avoid forcing legacy AES-CBC modes.
    [ValidateSet("Auto", "XtsAes128", "XtsAes256")]
    [string]$EncryptionMethod = "Auto",
...
Process {
    ...
    # CHANGES MADE:
    # Helper that builds Enable-BitLocker parameters and omits -EncryptionMethod when Auto is selected.
    function Get-EnableBitLockerSplat {
        param(
            [parameter(Mandatory = $true)]
            [string]$MountPoint,

            [parameter(Mandatory = $true)]
            [ValidateSet("Tpm", "RecoveryPassword")]
        ...
        switch ($ProtectorType) {
            "Tpm" { $splat["TpmProtector"] = $true }
            "RecoveryPassword" { $splat["RecoveryPasswordProtector"] = $true }
        }

        # "Auto" => do not pass EncryptionMethod. Let OS/MDM/GPO pick the modern default.
        if ($script:EncryptionMethod -ne "Auto") {
            $splat["EncryptionMethod"] = $script:EncryptionMethod
        }
    ...
    if (-not [System.Environment]::Is64BitProcess) {
        ...
        switch ($OperationalMode) {
            "Encrypt" {
                ...
                # CHANGES MADE: log whether we're forcing a method or letting policy decide
                if ($EncryptionMethod -eq "Auto") {
                    Write-LogEntry -Value "Encryption method set to Auto. Will omit -EncryptionMethod and allow OS/MDM/GPO policy to select the method (recommended)." -Severity 1
                }
                else {
                    Write-LogEntry -Value "Encryption method explicitly set to: $($EncryptionMethod)" -Severity 1
                }
                        ...
                        if ($BitLockerOSVolume -ne $null) {
                            ...
                            if (($BitLockerOSVolume.VolumeStatus -like "FullyDecrypted") -or ($BitLockerOSVolume.KeyProtector.Count -eq 0)) {
                                ...
                                try {
                                    ...
                                    # CHANGES MADE: use splat that omits -EncryptionMethod when Auto
                                    $Splat = Get-EnableBitLockerSplat -MountPoint $BitLockerOSVolume.MountPoint -ProtectorType "Tpm"
                                    Enable-BitLocker @Splat
                                }
                                catch [System.Exception] {
                                ...
                                try {
                                    ...
                                    # CHANGES MADE: use splat that omits -EncryptionMethod when Auto
                                    $Splat = Get-EnableBitLockerSplat -MountPoint $BitLockerOSVolume.MountPoint -ProtectorType "RecoveryPassword"
                                    Enable-BitLocker @Splat
                                }
                                catch [System.Exception] {
